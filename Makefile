...
PYPI_PACKAGES_PIPX = tmuxp pdm sci-hub ipython pix2tex
...
compare-packages: create-clean-pkglist
	...
	@diff -y --suppress-common-lines --color pkglist_clean.tmp <(pacman -Qqe | grep -vE "(paru|evdi-git|evdi-compat-git|wlroots-debug|cmake)" | sort) || echo ""
	@rm -f *tmp
