...
(use-package cape
  ...
  (advice-add 'pcomplete-completions-at-point :around #'cape-wrap-properties)
...
(add-hook 'org-agenda-finalize-hook #'my/org-agenda-colorize-s-d-prefix)
  ...
  (defun my/get-org-scheduled-time-range (s)
    "Transform `SCHEDULED: <DATE DAY HH:MM-HH:MM>` in string S to
   `SCHEDULED: <DATE DAY HH:MM>`."
    (when (stringp s)
          ...
          (org-map-entries
           (lambda ()
             (let* (
                    ...
                    (scheduled (my/get-org-scheduled-time-range (org-entry-get nil "SCHEDULED")))
...
see how ARG affects this command."
  ...
  (custom-set-faces
   ...
   '(org-code ((t (:background "#e0e0e0" :foreground "#000000" :inverse-video nil  :height 0.9 :box (:line-width 1 :color "#000000")))))
